...
from ppocr.utils.storage import storage_env
import tools.program as program
import matplotlib.pyplot as plt

def save_result(image_path, label, path):
    ...
    mlflow.log_artifact(path, artifact_path="rec_results")
...
def main():
    ...
    with open(save_res_path, "w") as fout:
        for file in get_image_file_list(infer_imgs, infer_list=infer_list):
            ...
            tensor_images = paddle.to_tensor(images)
            if config["Architecture"]["algorithm"] == "SRN":
                preds = model(tensor_images, others)
            elif config["Architecture"]["algorithm"] == "SAR":
                preds = model(tensor_images, img_metas)
            elif config["Architecture"]["algorithm"] == "RobustScanner":
                preds = model(tensor_images, img_metas)
            elif config["Architecture"]["algorithm"] == "CAN":
                preds = model([tensor_images, image_mask, label])
            else:
                preds = model(tensor_images)
            post_result = post_process_class(preds)
            # print(file.split("/")[-1])
            save_res_path = (
                    os.path.dirname(config["Global"]["save_res_path"]) + "/rec_results/"
                )
            if not os.path.exists(os.path.dirname(save_res_path)):
                os.makedirs(os.path.dirname(save_res_path))

            save_file = save_res_path + file.split("/")[-1]
    
            save_result(image_path=file, label=post_result, path=save_file)
